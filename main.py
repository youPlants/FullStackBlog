...
#Initializes jinja templates and sets up autoescape for html to reduce risks with entered text
template_dir = os.path.join(os.path.dirname(__file__), 'templates')
jinja_env = jinja2.Environment(loader = jinja2.FileSystemLoader(template_dir), autoescape=True)

#Secret helps enhance complexity of sha-256 encryption to make user data more secure
secret = '4gn-0-=x%7*@JMN-+[{78nskc83BHJH%$#_:>ngfrkvmntrf>*~HG'

#Helper function that takes the template and parameters to render templates, reduces redundant code
...
#Function takes in user's password encrypts it and checkes agains password hash in database
def check_secure_val(secure_val):
	val = secure_val.split('|')[0]
	if secure_val == make_secure_val(val):
		return val
#Main Handler that sets up basic webapp2 base functions to be inherited by page handlers
...
#Render function used by handlers, reduces typing necessary to render a template.
...
#Makes password hash, checks if salt exists and creates password hash using sha-256
def make_pw_hash(name, pw, salt = None):
	if not salt:
		salt = make_salt()
	h = hashlib.sha256(name + pw + salt).hexdigest()
	return '%s,%s' % (salt, h)
#Function hashes user's entered password and checks it against hashed password in datastore
...
#Function takes a User's name and returns the User form the datastore
	...
	def by_name(cls, name):
		...
		return u
#Function takes the User input from signup and register's the new User in the datastore
	@classmethod
	def register(cls, name, pw, email = None):
		pw_hash = make_pw_hash(name, pw)
		return User(parent = users_key(), name = name, pw_hash = pw_hash, email = email)
#Function calls the valid_pw function when a user sign's in to validate their credentials
	@classmethod 
...
#Method takes a post subject and finds the matching post
	...
	def by_subject(cls, subject):
		...
		return s
#Method to render each blog post, takes a post object and populates post template
	def render(self):
...
#Check that user entered subject and content for the post
		if subject and content:
			post = Post(parent= blog_key(), subject=subject, author=author, content=content, likes=0)
			post.put()
			self.redirect('/blog/%s' % str(post.key().id()))
#Returns an error if user forgets to enter a subject or content for the blog post.
		else:
...
#Handler for users to edit existing blog posts. Gets the post information and populates editpost template
class EditPost(Handler):
	def get(self):
		if self.user:
			...
			key = db.Key.from_path("Post", int(post_id), parent=blog_key())
			post = db.get(key)
#Check if user name matches the author so that users can only edit their own posts
...
#Check that post is not blank, and put post			
			if subject and content:
				...
				post.put()
				self.redirect('/blog/%s' % str(post.key().id()))
#If the post is missing a subject or content will return an error message and re generate edit page				
			else:
				error = "Please enter subject and content please!"
				self.render('newpost.html', subject=subject, author=author, content=content, error=error)
...
class DeletePost(Handler):
	def get (self):
		if self.user:
			...
			key = db.Key.from_path("Post", int(post_id), parent=blog_key())
			post = db.get(key)
#Checks that user name matches the author name and renders delete page to confirm post deletion.
			if self.user.name == post.author:
...
#Post method deletes post from the database when user confirms delete.
	def post(self):
		if self.user:
			...
			key = db.Key.from_path("Post", int(post_id), parent=blog_key())
			post = db.get(key)
#Checks that username matches author's name for the post and deletes it from the datastore and renders delete success page
			if post and post.author == self.user.name:
				post.delete()
				self.render('deletesuccess.html')
#If user's name doesn't match author's name will not delete post and return to front blog page
			else:
				self.redirect('/blog')
...
#Post method, takes the comment data and check usere data puts comment to the datastore.
	def post(self):
		if self.user:
			post_id = self.request.get("post")
			key = db.Key.from_path("Post", int(post_id), parent=blog_key())
...
class EditComment(Handler):
	def get(self):
		if self.user:
			...
			post = db.get(pKey)
#Checks that comment exists and the user's name matches the comment author's name
...
#Returns an error and renders new comment page if user tries to edit a comment they didn't write
			else: 
...
#Post method checks user data and takes edited comment data and puts the changes to the datastore
	def post(self):
		if self.user:
			comment_id = self.request.get("comment")
...
class DeleteComment(Handler):
	def get (self):
		if self.user:
			...
			pKey = db.Key.from_path("Post", int(post_id), parent=blog_key())
			post = db.get(pKey)
#Check that comment exists and that user's name matches the comment author's name
			if comment and self.user.name == comment.author:
				self.render('deletecomment.html', subject=post.subject, content=post.content, comment=comment.content)
#If user name doesn't match author's name or comment doesn't exist redirects to the front blog page
			else:
				self.redirect('/blog')
...
#Check that comment exists and that the author's name matches the user's name
			if comment and comment.author == self.user.name:
				comment.delete()
				self.render('deletesuccess.html')
#If comment doesn't exist or user doesn't match author return the front blog page.
			else:
				self.redirect('/blog')
#If user is not registered user redirect to the login page
		else:
			self.redirect('/login')
'''
Checks user credentials and handles post likes. Each user can only like a post from another author, amd can only like
each post once. Handler c
'''
...
#Checks if user is not author and if user has already liked the post yet
			if author == username or username in post.liked:
				self.redirect('/blog')
#If user hasn't written or liked post then puts 1 more like and username is added to the post's liked list
			else:
				post.likes += 1
...
#If user is not signed in then returns to login page
			...
			self.redirect('/login')
#Handler generates personalized message to welcome new user to the blog and confirm registration success
...
#Function validates that Username is contains letters or numbers and is 3-20 characters in length
...
#Handler for new user to signup with a unique username, matching passwords and valid email (optional)
class Signup(Handler):
	def get(self):
		self.render("signup.html")
#Takes user input and validates username is unique and that username, password, and email entereed are valid
	def post(self):
		have_error = False
...
#Takes user entered username, and checks if its valid using valid_username function
		if not valid_username(self.username):
			params['error_username'] = "That is not a valid username."
			have_error = True
#Takes user entered password, and passes it to valid_password function returns error message if invalid
		if not valid_password(self.password):
			params['error_password'] = "That is not a valid password"
			have_error = True
#Checks that user's two entered passwords match and returns an errror if they don't
		elif self.password != self.verify:
			params['error_verify'] = "Oops your passwords didn't match."
			have_error = True
#Takes entered email and passes it to valid_email to check if it is a valid email
		if not valid_email(self.email):
			params['error_email'] = "That's not a valid email address"
#If there is an error, will re render the signup form with any valid user data and the appropriate error message(s)
		if have_error:
			self.render('signup.html', **params)
		else:
			self.done()
#Method finishes validation, and returns error if not properly overwriten with register handler.
	def done(self, *a, **kw):
		raise NotImplementedError
#Overwrites the done function from signup class, checks that user is unique and puts new user to the datastore
...
#User name is not found in the database, puts new user's data to datastore
			...
			u.put()
#Logs in the newly registered user and redirects to the personalized welcome page
			self.login(u)
			self.redirect('/welcome')
#Handles user login, checks user's name and password hash against datastore and stores user's id in secure cookie
class Login(Handler):
	def get(self):
		self.render('login.html')
#Takes user's credentials and stores them in variables to check against datastore
	def post(self):
...
#If credentials match, log user in and redirect to front blog page
		if u:
			self.login(u)
			self.redirect('/blog')
#If credentials do not match datastore entry return error message and redirect to login page.
		else:
			msg = 'Invalid login'
			self.render('login.html', error = msg)
#Logout handler, passes user into logout function, which logs user out and delete's the secure cookie
